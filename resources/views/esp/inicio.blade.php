    ...
    <div class="h-dvh container grid grid-rows-[auto_1fr]">
      ...
      <div class="w-full h-full p-0 md:py-6 bg-background">
        <div class="w-full h-full text-background relative sm:rounded-3xl bg-hero bg-center bg-cover">
          <div class="flex flex-col px-6 md:px-8 py-10 sm:rounded-3xl gap-4 justify-center items-center text-center w-full h-full transition-all" x-data="{ isHovered: false }" x-bind:class="{ 'backdrop-blur-md bg-foreground/20' : isHovered }">
            ...
            <p class="sm:text-lg text-sm px-2 text-secondary text-center"></p><img class="sm:max-w-sm max-w-64 hover:rotate-x-12 hover:-translate-y-5 hover:scale-105 hover:drop-shadow-[0px_0px_10px_var(--color-highlight)] transition-all ease-in-out duration-200" x-on:mouseover="if(window.innerWidth &gt;= 768) isHovered = true" x-on:mouseleave="if(window.innerWidth &gt;= 768) isHovered = false" x-on:click="if(window.innerWidth &lt;= 768) isHovered = !isHovered" x-on:click.outside="if(window.innerWidth &lt;= 768) isHovered = false" src="{{ Vite::asset('resources/images/corteza-white-green.png') }}" alt="Corteza Logo">
            <p class="sm:text-lg text-sm px-2 text-secondary text-center">“Proveemos lo necesario para proteger lo más importante”</p>
            <div class="flex flex-col items-center absolute bottom-6 left-1/2 transform -translate-x-1/2 cursor-pointer hover:-translate-y-1 hover:scale-110 hover:animate-none transition-transform animate-bounce"><a class="sm:text-lg" href="{{ route('tiny-houses') }}">Ver Modelos</a><i class="bx bx-chevron-down bx-md"></i></div>
          </div>
        </div>
      </div>
      <!-- Banner Begin-->
      <div class="w-full flex flex-col items-stretch content-center flex-wrap p-0 md:p-6 py-8 bg-background bg-center bg-no-repeat bg-cover">
        <div class="px-6 md:px-8 py-10 sm:rounded-4xl bg-highlight bg-center bg-no-repeat bg-cover h-full container">
          <div class="flex items-center justify-around gap-4"><span class="text-sm font-bold sm:text-xl text-background">Servicio</span><i class="bx bxs-leaf text-base sm:text-4xl text-background"></i><span class="text-sm font-bold sm:text-xl text-background">Calidad</span><i class="bx bxs-leaf text-base sm:text-4xl text-background"></i><span class="text-sm font-bold sm:text-xl text-background">Protección</span>
          </div>
        </div>
      </div>
      <!-- Banner End-->
